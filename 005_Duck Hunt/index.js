...
function run() {
    ...
    const duckElem = ducks.map(setupDuckElem);

    function shootDuck (event) {
        const aDuckElem = event.target;
        aDuckElem.style.transition = 'top 2s';
        aDuckElem.style.top = `${window.innerHeight}px`;

        clearInterval(aDuckElem.Interval);
        
        setTimeout(() => {
            document.body.removeChild(aDuckElem);
